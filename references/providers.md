...
The host may use the Exa Agent route with `data_sources: [{"provider":"fiber"}]` for one already identity-resolved person. This is a host-owned provider bridge, not a network capability of contact-brief. The host must:
